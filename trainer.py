...
def train(net, train_loader, val_loader, n_epochs, lr, model_dir, resume=False, pretrained=None):
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, sample in enumerate(t_loader, 0):
            ...
            input, input_masked, label_id, label_mask = input.to(device), input_masked.to(
                device), label_id.to(device), label_mask.to(device)

            optimizer.zero_grad()

            # L_unmasked: L_arc + lambda * L_ce
            output, embed1, embed2, mask = net(input, label_id)
            loss = criterion(output, label_id) + 0.1 * \
                criterion(mask * 0, label_mask)      # lambda = 0.1

            # L_masked: L_arc + lambda * L_ce
            output_m, embed1_m, embed2_m, mask_m = net(input_masked, label_id)
            loss += criterion(output_m, label_id) + 0.1 * \
                criterion(mask_m, label_mask)
        ...
        if metrics["FMR100"] < best_score:
            logging.info(
                "- Found new best FMR100: {}".format(metrics["FMR100"]))
            ...
            if patience == 0:
                ...
                rate_decrease /= 10
                optimizer = optim.SGD(
                    param, lr * rate_decrease, weight_decay=5e-4, momentum=0.9)
                logging.info(
                    "- New Learning Rate: {}".format(lr * rate_decrease))
            else:
                patience -= 1
...
if __name__ == '__main__':
    ...
    train_loader, val_loader = dataloader.create_dataloader(
    ...
    lr = 0.1
    logging.info(
        "Start training for {} epoch(s) with lr = {} ...".format(n_epochs, lr))
    train(net, train_loader, val_loader, n_epochs, lr,
          args.model_dir, args.resume, args.pretrained)
